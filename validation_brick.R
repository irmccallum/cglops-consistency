...
#gdalwarp(srcfile=r,dstfile="c://CGLOPS/lc100_ag.tif",tr=c(0.01,0.01),r="mode",overwrite = TRUE)
#r <- raster("lc100_ag.tif")
#names(r) <- "code"
...
# create blank raster for rasterize
blank <- raster()
extent(blank) <- extent(c)
res(blank) <- res(c)
blank[] <- NA

# rasterize hotspot map
sr <- rasterize(s.map, blank, field = "hotspID", fun = 'modal')
...
p <- ggplot()+ 
  ...
  ggtitle(unique(s$aoi_name)[i]) 
ggsave(paste0("./results/",unique(s$aoi_name)[i],"AGREE.png",sep=""),  width=7.5, height=7.5, dpi=300)
